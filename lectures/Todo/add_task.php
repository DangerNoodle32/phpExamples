...
$dbc = mysqli_connect('localhost', 'root', 'password', 'php_task');
//$dbc = new mysqli('localhost', 'root', '', 'sem7');
...
if (($_SERVER['REQUEST_METHOD'] == 'POST') && !empty($_POST['task'])) {
    ...
    // $r = $dbc->query($q);
    // if (!$r) die ("Database access failed: " . $dbc->error);
...
$q = 'SELECT task_id, parent_id, task FROM tasks WHERE date_completed IS NULL ORDER BY date_added ASC';
$r = mysqli_query($dbc, $q);
